# Poketee — Testing Guide

Everything you need to test Poketee, from "see it on my laptop in 10 seconds" to
...
2. Find your computer's local IP:
   - macOS: **System Settings → Wi-Fi → Details → IP Address** (e.g. `192.168.1.20`)
   - Windows: run `ipconfig`, use the **IPv4 Address**
3. On your phone (same Wi-Fi) open: `http://YOUR-IP:8080/index.html?app=1`
4. Browser menu → **Add to Home Screen**. It installs as a Poketee icon and opens
...
This is how you give the app to other people to test. Cross-platform — do iOS,
Android, or both.
...
4. Go to **appstoreconnect.apple.com → your app → TestFlight**.
...
2. **Build → Generate Signed Bundle / APK → Android App Bundle (.aab)**. Create a
   keystore and **save it + its passwords** (you reuse it for every update).
3. Go to **play.google.com/console → your app → Testing → Internal testing →
   Create release**, upload the `.aab`.
...
> bigger step — see section 4.)

---

## 3. QA checklist — what to actually test in the app

Walk through these on each device:

**Lock / auth**
- [ ] First launch shows **Create a passcode**; entering 4 digits → **Confirm** → same
      4 digits unlocks to Capital home.
- [ ] Wrong confirm digits → shake + restart. 
- [ ] Close & reopen the app (cold start) → asks to **Enter passcode**; correct code
      unlocks; **Face ID** icon unlocks instantly (mock).
- [ ] Profile → **Sign out** → returns to the lock screen.

**Capital home**
- [ ] Balance shows; eye icon hides/reveals amounts.
- [ ] **Top up**: enter amount → choose source (bank / mobile money) → review → green
      success → **Back to Capital**.
...
- [ ] Company membership is right (Mfg&Ops: Blessed Dairy, Great Lakes Logistics •
      Services: Maran Design, TPNN, EXP.AFRICA • REIT: Shine Group, Heza Estate,
      Savannah Creek).
...
**Native behaviour**
- [ ] App fills the screen — no simulated phone frame, content clears the notch and
      home indicator.
- [ ] **Offline test:** enable Airplane Mode and relaunch — the app still opens and
      works (data is bundled).
- [ ] **Persistence:** navigate somewhere, force-quit, reopen → returns to where you
      were; passcode still set.
- [ ] Portrait orientation throughout; buttons reachable with a thumb.

**Push (only after you wire it — see README)**
...
| Give it to testers, iOS | Section 2 → TestFlight |
| Give it to testers, Android | Section 2 → Play Internal Testing |
